...
ANDI	R3, R30, 65535
;Flash_R_W.c,99 :: 		NVMCON = nvmop & 0x00004007;
ANDI	R2, R25, 16391
ANDI	R2, R2, 65535
SW	R2, Offset(NVMCON+0)(GP)
<<<<<<< HEAD
=======
;Flash_R_W.c,102 :: 		Delay_ms(50);
LUI	R24, 20
ORI	R24, R24, 22612
L_NVMUnlock3:
ADDIU	R24, R24, -1
...
NOP	
NOP	
;Flash_R_W.c,104 :: 		NVMKEY = 0xAA996655;
>>>>>>> temp
LUI	R2, 43673
ORI	R2, R2, 26197
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,105 :: 		NVMKEY = 0x556699AA;
LUI	R2, 21862
ORI	R2, R2, 39338
SW	R2, Offset(NVMKEY+0)(GP)
;Flash_R_W.c,108 :: 		NVMCONSET = 0x8000;
ORI	R2, R0, 32768
...
L_NVMUnlock3:
=======
;Flash_R_W.c,111 :: 		while (NVMCON & 0x8000);
L_NVMUnlock5:
>>>>>>> temp
; status start address is: 12 (R3)
...
=======
L_NVMUnlock6:
;Flash_R_W.c,114 :: 		if (status & 0x0001)
>>>>>>> temp
ANDI	R2, R3, 1
; status end address is: 12 (R3)
...
DI	R30
L_NVMUnlock8:
;Flash_R_W.c,120 :: 		NVMCONCLR = 0x0004000;
>>>>>>> temp
ORI	R2, R0, 16384
SW	R2, Offset(NVMCONCLR+0)(GP)
;Flash_R_W.c,123 :: 		return (NVMCON & 0x3000);
LW	R2, Offset(NVMCON+0)(GP)
ANDI	R2, R2, 12288
;Flash_R_W.c,124 :: 		}
L_end_NVMUnlock:
...
; end of _NVMUnlock
_ReadFlashWord:
;Flash_R_W.c,126 :: 		unsigned long ReadFlashWord(){
ADDIU	SP, SP, -512
;Flash_R_W.c,135 :: 		ptr = (unsigned char*)(FLASH_Settings_VAddr);
; ptr start address is: 20 (R5)
LWR	R5, Offset(_FLASH_Settings_VAddr+0)(GP)
LWL	R5, Offset(_FLASH_Settings_VAddr+3)(GP)
;Flash_R_W.c,136 :: 		for(i=0;i<512;i++){
...
; Val start address is: 8 (R2)
ANDI	R2, R2, 255
;Flash_R_W.c,142 :: 		Val =(Val<<8)| buff[2];
SLL	R3, R2, 8
; Val end address is: 8 (R2)
ADDIU	R2, R4, 2
LBU	R2, 0(R2)
ANDI	R2, R2, 255
OR	R2, R3, R2
;Flash_R_W.c,143 :: 		Val =(Val<<8)| buff[1];
SLL	R3, R2, 8
ADDIU	R2, R4, 1
LBU	R2, 0(R2)
ANDI	R2, R2, 255
OR	R2, R3, R2
;Flash_R_W.c,144 :: 		Val =(Val<<8)| buff[0];
SLL	R3, R2, 8
LBU	R2, 0(R4)
ANDI	R2, R2, 255
OR	R2, R3, R2
;Flash_R_W.c,146 :: 		return Val;//0xafffafff;
;Flash_R_W.c,147 :: 		}
L_end_ReadFlashWord:
ADDIU	SP, SP, 512
JR	RA
